...
def softmax_loss(w0, b0, w1, b1, w2, b2, x, y):
    ...
    err = -np.sum(y * np.log(out))  # / x.shape[0]
    return err
...
nw0 = approx_fprime(w0.ravel(), lambda w: softmax_loss(w.reshape(w0.shape), b0, w1, b1, w2, b2, x, y), eta)
nb0 = approx_fprime(b0.ravel(), lambda b: softmax_loss(w0, b.reshape(b0.shape), w1, b1, w2, b2, x, y), eta)

nw1 = approx_fprime(w1.ravel(), lambda w: softmax_loss(w0, b0, w.reshape(w1.shape), b1, w2, b2, x, y), eta)
nb1 = approx_fprime(b1.ravel(), lambda b: softmax_loss(w0, b0, w1, b.reshape(b1.shape), w2, b2, x, y), eta)

nw2 = approx_fprime(w2.ravel(), lambda w: softmax_loss(w0, b0, w1, b1, w.reshape(w2.shape), b2, x, y), eta)
nb2 = approx_fprime(b2.ravel(), lambda b: softmax_loss(w0, b0, w1, b1, w2, b.reshape(b2.shape), x, y), eta)
