...
def plot_train_val_error_vs_epoch(errors_on_train, errors_on_test, title):
    import matplotlib.pyplot as plt

    epochs_val = len(errors_on_test)
    ...
    plt.plot(epochs2, errors_on_train, 'r',epochs, errors_on_test,'b')
...
def gradient_descent(learning_rate, hidden_u_l1, hidden_u_l2,
    ...
    while True:
        ...
        error_on_test = softmax_loss(w0,  b0, w1,b1, w2,  b2, test_inputs, test_labels)
        errors_on_test.append(error_on_test)

        title = "LR:_{0}_H1:_{1}_H2_{2}_EP_{3}:_Error_on_train:_{4}_on val:_{5}".format(learning_rate, hidden_u_l1, hidden_u_l2, epoch, error_on_train, error_on_test)
        print(title)
    ...
    return [w0, b0, w1, b1, w2, b2, epoch, errors_on_train, errors_on_test]
...
[w0, b0, w1, b1, w2, b2, epoch, errors_on_train, errors_on_test] = gradient_descent(min_lr, min_h1, min_h2, min_epochs, convergence_condition, min_error_conv)

# plot error vs epoch
plot_train_val_error_vs_epoch(errors_on_train, errors_on_test, "Error_rate_by_the_number_of_epochs_test")
